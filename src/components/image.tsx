import { createResource, mergeProps, splitProps, Suspense, Component, type JSX, type ComponentProps } from "solid-js";
...
const ImageComponent: Component<ImageProps> = (props) => {
  props = mergeProps({ src: [] }, props);
  const [custom, rest] = splitProps(props, ["src", "fallback"]);

  const imageResource = createImage(custom.src);
